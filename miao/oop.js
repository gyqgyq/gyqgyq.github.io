...
class Myset {
  constructor(value = []) {
    this._values = []
    value.forEach(this.add, this)//函数被取出来用,需要绑定this
  }
  get size() {
  ...
  forEach(iterator, thisArg) {
    for (let item of this._values) {
      iterator(item, item, this._values)
    }
  }
}
...
//全局有一个indexOf函数, 为了安全, 把整个代码放入闭包里
  ...
  MyMap.prototype.keyIndex = function(ary, tar) {
    if (tar !== tar) {
      for (let i = 0; i < ary.length; i++) {
        if (ary[i] !== ary[i]) {
          return i
  ...
  MyMap.prototype.get = function(key) {
    let idx = keyIndex(this._keys, key)
    if (idx >= 0) {
  ...
  MyMap.prototype.has = function(key) {
    return keyIndex(this._keys, key) >= 0
  }
...
class MyArray {
  construceor(len) {
    ...
    Object.defineProperty(this, 'length', {
      get: function() {
        ...
        maxIntIndex++
        if (maxIntIndex > this._legth) {
          this.length = maxIntIndex 
          return this.length
        } else {
          return this.length
        }
  ...
  push() {
    for (let i = this.length, j = 0; j < argumengths.length;) {
      this[i] = argumengths[j]
    }
    return this.length
  }
}
